...
| `--underperforming` | Pages with high traffic but poor engagement/conversion |
| `--events` | Event overview with counts for all tracked events |
...
- The `generate_lead` event is the primary lead conversion event (auto-tracked by GA4 Enhanced Measurement on form submissions)
