...
const cat = config.categories["service-requests"];

export default function ServiceRequestsCalculator({
  initialParams,
}: {
  ...
  const [requests, setRequests] = useState(paramNum(initialParams, "reqs", defaults.requestsPerMonth));
  ...
  useUrlSync("/branchen/grosshandel/roi-rechner/service-requests", {
    reqs: requests,
    before: minBefore,
    after: minAfter,
    hourly: hourlyCost,
  ...
  const results = useMemo(() => {
    const timeSavedPerReq = minBefore - minAfter;
    const totalHoursSaved = (requests * timeSavedPerReq) / 60;

    const costWithoutAi = (requests * minBefore / 60) * hourlyCost;
    const costWithAi = (requests * minAfter / 60) * hourlyCost;
    ...
    return { timeSavedPerReq, totalHoursSaved, costWithoutAi, costWithAi, netSavings, amortMonths, netYear1, roiYear1, netYear2, roiYear2 };
  }, [requests, minBefore, minAfter, hourlyCost, activeMrr, defaults]);
    ...
    >
        ...
        {isPositive ? (
          ...
          >
            ...
            <p className="text-lg font-semibold leading-relaxed">
              {requests.toLocaleString("de-DE")} Requests/Monat mit je{" "}
              {results.timeSavedPerReq} Min. Ersparnis bringen euch{" "}
          ...
          <div className="bg-red-50 rounded-2xl p-6 flex flex-col justify-center">
            ...
            <p className="text-lg font-semibold text-red-700 leading-relaxed">
              ...
              Erhöhe das Request-Volumen oder die Zeitersparnis pro Request.
            </p>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-2xl border border-gray-200 p-6 md:p-8 space-y-8 shadow-sm">
          ...
          <Slider label={sliders.requestsPerMonth.label} value={requests} onChange={setRequests} min={sliders.requestsPerMonth.min} max={sliders.requestsPerMonth.max} step={sliders.requestsPerMonth.step} />
